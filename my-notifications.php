...
if(isset($_GET['del']))
{
$query=mysqli_query($con,"UPDATE tbl_notify SET `status` =  2 WHERE id='$wid'");
}


if(isset($_GET['action']) && $_GET['action']=="add"){
	$id=intval($_GET['id']);
	$query=mysqli_query($con,"delete from tbl_wishlist where Product_Id='$id'");
...
<div class="body-content outer-top-bd">
	...
	<div class="table-responsive">
		<table class="table">
			<thead>
				<tr>
					<th colspan="4">my Notifications</th>
				</tr>
			</thead>
			<tbody>
<?php
$ret=mysqli_query($con,"select tbl_products.name as pname,tbl_products.id as proid,tbl_products.image as pimage,tbl_notify.Product_Id as pid,tbl_notify.update_status as ustatus,tbl_notify.id as nid from tbl_notify join tbl_products on tbl_products.id=tbl_notify.Product_Id where tbl_notify.UserId='".$_SESSION['userid']."' and tbl_notify.status=1");
...
?>
				...
				<tr>
					...
					<td class="col-md-6">
						<div class="product-name"><a href=""><?php echo htmlentities($row['pname']);?></a></div>
						<div class="price">
							<?php echo htmlentities($row['ustatus']);?>
						</div>
					</td>
					<td class="col-md-2 close-btn">
						<a href="my-notifications.php?del=<?php echo htmlentities($row['nid']);?>" onClick="return confirm('Are you sure you want to delete?')" class=""><i class="fa fa-times"></i></a>
					</td>
				</tr>
				<?php } } else{ ?>
				<tr>
					<td style="font-size: 18px; font-weight:bold ">Your have no new notifications</td>
