...
header('location:login.php');
	...
	if(isset($_POST['update']))
	{
		...
		$query=mysqli_query($con,"update tbl_users set Name='$name',Phone_No='$contactno' where User_Id'".$_SESSION['userid']."'");
		if($query)
		{
...
{
$sql=mysqli_query($con,"SELECT `Password` FROM  `tbl_users` where `Password`='".$_POST['cpass']."' && User_Id='".$_SESSION['userid']."'");
$num=mysqli_fetch_array($sql);
if($num>0)
{
 $con=mysqli_query($con,"update tbl_users set Password='".$_POST['newpass']."' where User_Id='".$_SESSION['userid']."'");
...
<div class="form-group">
			 ...
			 <input type="email" class="form-control unicase-form-control text-input" id="exampleInputEmail1" value="<?php echo $row['Email'];?>" readonly>
					  ...
					  <div class="form-group">
					    <label class="info-title" for="Contact No.">Phone No. <span>*</span></label>
					    <input type="text" class="form-control unicase-form-control text-input" id="contactno" name="contactno" required="required" value="<?php echo $row['Phone_No'];?>"  maxlength="10">
					  </div>
...
<div class="form-group">
			...
			<?php include('project/includes/myaccount-sidebar.php');?>
			</div><!-- /.row -->
		</div><!-- /.checkout-box -->
