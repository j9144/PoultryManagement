...
$query=mysqli_query($con,"insert into tbl_users(Name,Email,Phone_No,Password) values('$name','$email','$contactno','$password')");
if($query)
{
...
$status=1;
$log=mysqli_query($con,"insert into tbl_userlog(UserEmail,UserIp,Status) values('".$_SESSION['Email']."','$uip','$status')");
...
$uip=$_SERVER['REMOTE_ADDR'];
$status=0;
$log=mysqli_query($con,"insert into tbl_userlog(UserEmail,UserIp,Status) values('$email','$uip','$status')");
$host  = $_SERVER['HTTP_HOST'];
$uri  = rtrim(dirname($_SERVER['PHP_SELF']),'/\\');
header("location:http://$host$uri/$extra");
...
<html lang="en">
	<head>
	    ...
	    <title>Madayikkal Poultries | Sign-in | Signup</title>
