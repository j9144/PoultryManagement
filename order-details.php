...
<?php
$ret = mysqli_query($con,"select t.Email,t.Order_Id from (select usr.Email,odrs.Order_Id from tbl_users as usr join tbl_orders as odrs on usr.User_Id=odrs.User_Id) as t where  t.Email='$email' and (t.Order_Id='$orderid')");
$num=mysqli_num_rows($ret);
if($num>0)
{
$query=mysqli_query($con,"select tbl_products.name as pname,tbl_products.image as pimg,tbl_products.id as proid,tbl_orders.Product_Id as opid,tbl_orders.Quantity as qty,tbl_products.productPrice as pprice,tbl_orders.PaymentMethod as paym,tbl_orders.OrderDate as odate,tbl_orders.Order_Id as orderid from tbl_orders join tbl_products on tbl_orders.Product_Id=tbl_products.id where tbl_orders.Order_Id='$orderid' and tbl_orders.PaymentMethod is not null");
$cnt=1;
