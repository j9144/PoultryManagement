...
Class Action {
	...
	function save_store(){
		...
		if(empty($id)){
			...
			while($i == 0){
				...
				$chk = $this->db->query("SELECT * FROM tbl_store where Store_code = $bcode")->num_rows;
				if($chk <= 0){
					$i = 1;
				}
			}
			$data .= ", Store_code='$bcode' ";
			$save = $this->db->query("INSERT INTO tbl_store set $data");
		}else{
			$save = $this->db->query("UPDATE tbl_store set $data where Store_Id = $id");
		}
		if($save){
			return 1;
		}
	...
	function delete_store(){
		extract($_POST);
		$delete = $this->db->query("DELETE FROM tbl_store where Store_Id = $id");
		if($delete){
			return 1;
		}
	}
